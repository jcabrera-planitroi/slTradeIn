...
public class ManualQuoteController : CustomController
{
    ...
    public ManualQuoteController(Detail_TTU_userManualQuote_Data detail_TTU_UserManualQuote_Data,
        Detail_TTU_userCartDetail_Data detail_TTU_UserCartDetail_Data,
        Detail_TTU_userManualQuoteDetail_Data detail_TTU_UserManualQuoteDetail_Data,
        Detail_TTU_userManualQuoteLocation_Data detail_TTU_UserManualQuoteLocation_Data)
    {
    ...
    {
        var cartID = int.Parse(SessionTradeIn.iCartID);
    ...
    public ActionResult Edit()
    {
        ...
        if (Q == null)
            Model = new ManualQuoteModel
            {
                ...
                ListDetail =
                    new List<Detail_TTU_userManualQuoteDetail>(_detail_TTU_UserManualQuoteDetail_Data.GetList(0)),
                ListLocation = new List<ManualQuoteLocationModel>(
                    _detail_TTU_UserManualQuoteLocation_Data.GetList(0, Convert.ToInt32(SessionTradeIn.iUserID)))
            };
        else
            Model = new ManualQuoteModel
            {
                ...
                ListDetail =
                    new List<Detail_TTU_userManualQuoteDetail>(
                        _detail_TTU_UserManualQuoteDetail_Data.GetList(Q.iManualQuoteID)),
                ListLocation = new List<ManualQuoteLocationModel>(
                    _detail_TTU_UserManualQuoteLocation_Data.GetList(Q.iManualQuoteID,
                        Convert.ToInt32(SessionTradeIn.iUserID)))
            };
    ...
    public ActionResult Edit(ManualQuoteModel model)
    {
        ...
        model.ListLocation = new List<ManualQuoteLocationModel>(
            _detail_TTU_UserManualQuoteLocation_Data.GetList(model.iManualQuoteID,
                Convert.ToInt32(SessionTradeIn.iUserID)));
        foreach (var item in model.ListLocation)
        {
            item.iManualQuoteID = model.iManualQuoteID;
            if (Request.Form.ContainsKey("chk_" + item.iLogisticID))
                item.bCheck = true;
            else
                item.bCheck = false;
        }
